...
export default function ExitIntentPopup() {
  ...
  useEffect(() => {
    ...
    // SI ESTAMOS A FINALES DE MES (día 20 o más), OFRECEMOS EL PRÓXIMO MES
    if (targetDate.getDate() >= 20) {
      targetDate.setMonth(targetDate.getMonth() + 1);
    }
    setCurrentMonth(new Intl.DateTimeFormat('es-ES', { month: 'long' }).format(targetDate));
    
    // Solo mostramos en desktop cuando el mouse sale hacia arriba
  ...
  const handleClaim = () => {
    window.open(getWhatsAppUrl(`Hola, me gustaría reservar aprovechando la promoción del 10% de descuento para todo el mes de ${currentMonth}.`), "_blank");
    setIsOpen(false);
  };
